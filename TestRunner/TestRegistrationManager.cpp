...
TEST_CASE("RegistrationManager: Register and Unregister Task")
{
    ...
    auto verifyManagerEntries = [&](const auto &entries, const auto &expected_entry, size_t expected_size, bool expected_result)
    {
        bool found = false;
    ...
    auto verifyHandlerEntries = [&](const auto &handlers, const auto &task, CyphalPortID port_id, size_t expected_size, bool expected_result)
    {
        bool found = false;
