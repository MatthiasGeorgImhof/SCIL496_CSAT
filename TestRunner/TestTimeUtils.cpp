...
TEST_CASE("RTC <-> DateTimeComponents Conversions") {
  ...
  TimeUtils::RTCDateTimeSubseconds rtc_datetime =
      TimeUtils::to_rtc(components, secondFraction);
...
TEST_CASE("Comprehensive Round Trip Test") {
  constexpr uint32_t secondFraction = 1023;
  TimeUtils::DateTimeComponents initial_components = {2025, 5, 20, 8, 15, 30,
                                                      250};
  ...
  CHECK(abs(static_cast<int>(final_components.millisecond) -
            static_cast<int>(initial_components.millisecond)) <
        20);  // Tolerance for millisecond differences
}
