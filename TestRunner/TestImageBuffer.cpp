...
TEST_CASE("ImageBuffer - Add and Read Image - Static Access") {
    ...
    REQUIRE(buffer.add_image(image_data.data(), metadata) == 0);
...
TEST_CASE("ImageBuffer small - Add, Add, Drop, and Read Image") {
    // Define the memory configuration for this test
    uint32_t flash_start = 0x08000000;
    uint32_t total_size = 1024 * 100;
...
TEST_CASE("ImageBuffer large - Add, Add, Drop, and Read Image") {
    // Define the memory configuration for this test
    uint32_t flash_start = 0x08000000;
    uint32_t total_size = 1024 * 100;
    ...
    // Create a sample image
    std::vector<uint8_t> image_data = std::vector<uint8_t>(50000, 0xAA);
    size_t image_size = image_data.size();
...
// //     auto status = buffer.add_image(image_data.data(), metadata);
// //     REQUIRE(status == 0);
...
//     ImageMetadata metadata;
//     metadata.image_size = image_data.size();
//     buffer.add_image(image_data.data(), metadata);
...
//     REQUIRE(buffer.add_image(image_data1.data(), metadata1) == 0);
//     REQUIRE(buffer.add_image(image_data2.data(), metadata2) == 0);
...
//     auto result1 = buffer.add_image(image_data1.data(), metadata1);
//     REQUIRE(result1 == 0);
//     auto result2 = buffer.add_image(image_data2.data(), metadata2);
//     REQUIRE(result2 == 0);
...
//     REQUIRE(buffer.add_image(image_data3.data(), metadata3) == -1);

//     //Dropping first image will now make us add another
//     REQUIRE(buffer.drop_image() == true);
//     REQUIRE(buffer.add_image(image_data3.data(), metadata3) == 0);
...
//     ImageMetadata metadata2;
//     metadata2.image_size = image_data2.size();
//     REQUIRE(buffer.add_image(image_data2.data(), metadata2) == -1);
// }
...
//     REQUIRE(buffer.add_image(image_data.data(), metadata) == -1);
// }
