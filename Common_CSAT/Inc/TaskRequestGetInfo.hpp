...
class TaskRequestGetInfo : public TaskForClient<CyphalBuffer8, Adapters...>
{
public:
    TaskRequestGetInfo() = delete;
    TaskRequestGetInfo(uint32_t interval, uint32_t tick, CyphalNodeID node_id, CyphalTransferID transfer_id, std::tuple<Adapters...> &adapters) : TaskForClient<CyphalBuffer8, Adapters...>(interval, tick, node_id, transfer_id, adapters) {}
...
void TaskRequestGetInfo<Adapters...>::handleTaskImpl()
{
    if (TaskForClient<CyphalBuffer8, Adapters...>::buffer_.is_empty())
    {
        ...
        TaskForClient<CyphalBuffer8, Adapters...>::publish(PAYLOAD_SIZE, payload, &data,
    ...
    size_t count = TaskForClient<CyphalBuffer8, Adapters...>::buffer_.size();
