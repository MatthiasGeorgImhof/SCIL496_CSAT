...
template <typename BaseAccessor, size_t BLOCK_SIZE>
AccessorError BufferedAccessor<BaseAccessor, BLOCK_SIZE>::fill_cache(size_t address)
{
...
template <typename BaseAccessor, size_t BLOCK_SIZE>
AccessorError BufferedAccessor<BaseAccessor, BLOCK_SIZE>::write(size_t address, const uint8_t *data, size_t size)
{
    size_t data_offset = 0;
    while (size > 0)
    {
        size_t block_offset = address % BLOCK_SIZE;
        size_t block_start_address = address - block_offset;
        size_t block_end_address = block_start_address + BLOCK_SIZE - 1;
        size_t block_remaining_size = block_end_address - address + 1;
...
template <typename BaseAccessor, size_t BLOCK_SIZE>
AccessorError BufferedAccessor<BaseAccessor, BLOCK_SIZE>::read(size_t address, uint8_t *data, size_t size)
{
    size_t data_offset = 0;
    while (size > 0)
    {
...
namespace BUFFERED_ACCESSOR_H
{
    ...
    public:
        ...
        AccessorError write(size_t /*address*/, const uint8_t */*data*/, size_t /*size*/) { return AccessorError::NO_ERROR; };
        AccessorError read(size_t /*address*/, uint8_t */*data*/, size_t /*size*/) { return AccessorError::NO_ERROR; };
