		...
		float &xfact, float &xlamo, float &xli, float &xni)
	{
		...
		if (irez != 0)
		{
			...
			if (irez == 2)
			{
				...
				f522 = 9.84375f * sinim * (sini2 * (1.0f - 2.0f * cosim - 5.0f * cosisq) + 0.33333333f * (-2.0f + 4.0f * cosim + 6.0f * cosisq));
				f523 = sinim * (4.92187512f * sini2 * (-2.0f - 4.0f * cosim + 10.0f * cosisq) + 6.56250012f * (1.0f + 2.0f * cosim - 3.0f * cosisq));
				f542 = 29.53125f * sinim * (2.0f - 8.0f * cosim + cosisq * (-12.0f + 8.0f * cosim + 10.0f * cosisq));
				f543 = 29.53125f * sinim * (-2.0f - 8.0f * cosim + cosisq * (12.0f + 8.0f * cosim - 10.0f * cosisq));
				xno2 = nm * nm;
				ainv2 = aonv * aonv;
	...
	*    vallado, crawford, hujsak, kelso  2006
		...
		float r[3], float v[3])
	{
		...
		else
		{
			...
			rvdot = rvdotl + nm * temp1 * (satrec.x1mth2 * cos2u + 1.5f * satrec.con41) / satrec.xke;
