...
int main() {
    ...
    float startmfe, stopmfe, deltamin;
    ...
    elsetrec satrec;
    memset(&satrec, 0, sizeof(elsetrec)); // Initialize the structure to zero
    ...
    float start_time = 0.0; // Start time in minutes past epoch
    float end_time = 60.0;   // End time in minutes past epoch (e.g., 60 minutes)
    float step_size = 5.0;  // Time step in minutes (e.g., predict every 5 minutes)

    // 6. Loop through the time span and predict the satellite's position
    for (float tsince = start_time; tsince <= end_time; tsince += step_size) {
        float r[3]; // Position vector (km)
        float v[3]; // Velocity vector (km/s)
