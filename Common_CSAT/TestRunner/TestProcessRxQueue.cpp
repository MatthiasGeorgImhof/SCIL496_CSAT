...
TEST_CASE("CanProcessTxQueue with CanardAdapter")
{
    ...
    transfer.metadata.remote_node_id = CYPHAL_NODE_ID_UNSET;
    transfer.metadata.transfer_id = 0;
    transfer.payload_size = 10;
    char payload[11] = "helloehllo";
    transfer.payload = payload;
    ...
    CHECK(cyphal.cyphalTxPush(0, &transfer.metadata, transfer.payload_size, transfer.payload) == 2);
    CHECK(canard_adapter.que.size == 2); // Ensure the queue has been emptied.
    ...
    CHECK(canard_adapter.que.size == 0); // Ensure the queue has been emptied.
    CHECK(get_can_tx_buffer_count() == 2); //Check tx buffer count.
}