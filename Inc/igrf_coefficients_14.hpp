...
namespace igrf_model {

// static constexpr std::array igrfCoefficients = {
...
// constexpr size_t N_COFS = igrfCoefficients.size();
// constexpr int getN(const GaussCoefficient& gc) { return gc.n; }
// static constexpr int NMAX = std::ranges::max(igrfCoefficients | std::views::transform(getN));
// static_assert((NMAX+1) * (NMAX+2) / 2 - 1 == N_COFS, "NMAX does not align with number of coefficients");
// static_assert(14 == N_COFS, "Number of coefficients != 14");
