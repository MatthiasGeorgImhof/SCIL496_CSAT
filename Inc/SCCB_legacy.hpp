#ifndef __SCCB_LEGACY__HPP
#define __SCCB_LEGACY__HPP
...
std::vector<uint8_t> SCCB_SDA_W, SCCB_SDA_R, SCCB_SCK_W;
...
static inline uint8_t SCCB_SDA_Read(void) { return HAL_GPIO_ReadPin(SCCB_SDA_PORT, SCCB_SDA_PIN); }
...
void SCCB_WriteReg8(uint8_t dev, uint16_t reg, uint8_t value)
{
    SCCB_SDA_AsOutputOD();
...
uint8_t SCCB_ReadReg8(uint8_t dev, uint16_t reg)
{
    uint8_t value;
